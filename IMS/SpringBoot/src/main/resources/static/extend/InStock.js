...
app.controller('personCtrl', function ($scope, $http, $location) {
    ...
    window.onload = function () {
        ...
        $scope.operationNo = Request.operationNo != null ? Request.operationNo.split("#")[0] : "";
        //获得登陆用户
        $http({
            method: 'POST',
            url: '/getUser',
        ...
        $http({
            ...
            $scope.stockName = rs.data.stockName
            $scope.creater = rs.data.creater;
...
function initOrderExtendTable($http, $scope) {
    ...
    if ($scope.orderNo != "") {
        $http({
            method: 'POST',
            url: '/Operation/getOperationDetail',
            data: {
                "operationNo": $scope.operationNo
            },
