...
app.controller('personCtrl', function ($scope, $http, $interval, $compile) {
    ...
    $scope.checkOrder = function () {
        ...
        if (confirm("是否确认审核通过该订单？审核后货品将计入仓库")) {
            $http({
                method: 'POST',
                url: 'order/checkOrdersOut',
                params: {
                    ids: ids
                },
            }).then(function successCallback(rs) {
    ...
    //编辑点击事件
    $('#order-table tbody').on('click', '.editEx', function () {
        var table = $('#order-table').DataTable();
        var nowRow = table.row($(this).parents('tr')).data();
        window.location.href = "orderExtendOutMange?orderNo=" + nowRow[2];
    });
    //编辑点击事件
    $('#order-table tbody').on('click', '.viewEx', function () {
        var table = $('#order-table').DataTable();
        var nowRow = table.row($(this).parents('tr')).data();
        window.location.href = "orderExtendView?orderNo=" + nowRow[2];
    });
    ...
    $http({
        ...
        for (var i = 0; i < result.data.length; i++) {
            table.row.add([checkBox,
                ...
                result.data[i].orderTypeName,
                result.data[i].orderStatusName,
                result.data[i].orderCost,
